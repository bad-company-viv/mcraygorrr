        ...
        <div className="pt-32 pb-24 bg-white min-h-screen relative overflow-hidden">
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="max-w-5xl mx-auto">
                    ...
                    {/* Other Projects Linking Section */}
                    <div className="mt-24">
                        <div className="flex items-center justify-between mb-10">
                            <h3 className="text-2xl font-black text-slate-900 tracking-tight">Other Featured Projects</h3>
                            <Link href="/projects" className="text-sm font-black text-orange-600 uppercase tracking-widest hover:translate-x-1 transition-transform inline-flex items-center gap-2">
                                View All <ArrowRight className="w-4 h-4" />
                            </Link>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {projects.filter(p => p.id !== projectId).slice(0, 2).map((otherProject) => (
                                <Link key={otherProject.id} href={`/projects/${otherProject.id}`} className="group">
                                    <div className="flex gap-6 items-center bg-slate-50 p-6 rounded-[2rem] border border-slate-100 hover:border-orange-500/20 hover:shadow-xl transition-all duration-500">
                                        <div className="w-24 h-24 flex-shrink-0 rounded-2xl overflow-hidden shadow-sm">
                                            <img
                                                src={otherProject.image.src || otherProject.image}
                                                alt={otherProject.title}
                                                className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                                            />
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-orange-600 font-black uppercase tracking-widest mb-1">{otherProject.category}</p>
